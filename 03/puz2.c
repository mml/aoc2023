...
long find_sum();
long find_ratio(int32_t, int32_t);
bool is_symbol(char);
bool is_digit(char);
...
long find_ratio(int32_t y, int32_t x) {
  int32_t i,j,lo;
  long rands[2];
  int32_t found = 0;

  for (i = max(0, y-1); i <= min(rows-1, y+1); i++) {
    char *line = get_line(inbuf, i);
    for (j = max(0, x-1); j <= min(columns-1, x+1); j++) {
      if (is_digit(line[j])) {
        if (2 == found)
          return 0; // too many
      } else {
      ...
      rands[found++] = strtol(&line[lo], NULL, 10);
      if (LONG_MIN == rands[found-1] || LONG_MAX == rands[found-1])
        err(EX_SOFTWARE, "strtol");
    }
  }
